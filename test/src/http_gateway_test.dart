...
TickflowChatConfig config({TokenProvider? token, required http.Client client}) =>
    TickflowChatConfig(
...
void main() {
  test('null token short-circuits to ChatAuthException without network', () async {
    var calls = 0;
    ...
    await expectLater(gateway.createSession(), throwsA(isA<ChatAuthException>()));
    expect(calls, 0);
  });
  ...
  test('401 with an unchanged token fails fast as expired (no second call)', () async {
    var calls = 0;
    final gateway = HttpGateway(config(
      token: () async => 'same',
      client: MockClient((req) async {
    ...
    await expectLater(gateway.createSession(), throwsA(isA<ChatAuthException>()));
    expect(calls, 1, reason: 'no refresh exists — retrying the same token is pointless');
  });
  ...
  test('404 maps to ChatNotFoundException', () async {
    final gateway = HttpGateway(config(
      client: MockClient((req) async => http.Response('{"error":"session gone"}', 404)),
    ));
    await expectLater(gateway.createSession(), throwsA(isA<ChatNotFoundException>()));
  });
  ...
  test('transport-level failure maps to ChatNetworkException', () async {
    ...
    await expectLater(gateway.createSession(), throwsA(isA<ChatNetworkException>()));
  });
  ...
  test('openMessageStream posts the turn and yields parsed events', () async {
    ...
    expect(jsonDecode(body), {'sessionId': 's1', 'text': 'hello', 'clientTag': 'tag-1'});
    expect((events[0] as TokenDelta).delta, 'Hi');
    expect(events[1], isA<StreamDone>());
  });

  test('openMessageStream maps a non-200 before any event is emitted', () async {
    final gateway = HttpGateway(config(
      client: MockClient.streaming((req, bodyStream) async => http.StreamedResponse(
